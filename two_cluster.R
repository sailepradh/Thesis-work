...
k2 <- kmeans(comp2, 2, nstart=25, iter.max=1000)
library(RColorBrewer)
library(scales)
palette(alpha(brewer.pal(9,'Set1'), 0.5))
plot(comp2, col=k2$clust, pch=16)

# 3D plot

plot3d(comp2$PC1,comp2$PC2, comp2$PC3, col=k2$clust)
...
# First cluster
row.names(my_data_2[k2$clust==clust[1],])
# Second Cluster
row.names(my_data_2[k2$clust==clust[2],])
...
#################################################################################################
#################################################################################################
TPK_cluster <- kmeans ((my_data_2 [,1:2]), 2)
TPK_cluster
...
sort(table(TPK_cluster$clust))
clust_2 <- names(sort(table(TPK_cluster$clust)))

row.names(my_data_2[TPK_cluster$clust==clust_2[1],])
row.names(my_data_2[TPK_cluster$clust==clust_2[2],])

plot(my_data_2[,1],my_data_2[,2], col=TPK_cluster$cluster, pch =16, 
     xlab="TPK_Baseline", ylab="TPK_Nadir", main="Clustering on 2 clustering in TPK phenotype")
points(TPK_cluster$centers, col =1:2, pch =15, cex=3)
text (my_data_2[,1],my_data_2[,2],labels  = Phe_WGS$TPK_CTC, pos = 3,cex=0.75)
Clustered_ID_TPK<- as.data.frame(cbind(TPK_cluster$cluster, as.character(Phe_WGS$FID)))
Clustered_ID_TPK$Group <- ifelse(Clustered_ID_TPK$V1==1,"LowTox","HighTox")
Clustered_ID_TPK$WGS_sam <- which(Clustered_ID_TPK$V2 %in% WGS_sample$V2)
Clustered_ID_TPK$V2 <- as.character(Clustered_ID_TPK$V2)
Clustered_ID_TPK$WGS_sam <- WGS_Sample_Reordered[which(Clustered_ID_TPK$V2 %in% WGS_Sample_Reordered$V2),][,1]



#################################################################################################
#################################################################################################
LPK_cluster <- kmeans ((my_data_2 [,3:4]), 2)
LPK_cluster
...
points(LPK_cluster$centers, col =1:2, pch =15, cex=3)
text (my_data_2[,3],my_data_2[,4],labels  = Phe_WGS$LPK_CTC, pos = 3,cex=0.75)

sort(table(LPK_cluster$clust))
clust_3 <- names(sort(table(LPK_cluster$clust)))

row.names(my_data_2[LPK_cluster$clust==clust_3[1],])
row.names(my_data_2[LPK_cluster$clust==clust_3[2],])


Clustered_ID_LPK<- as.data.frame(cbind(LPK_cluster$cluster, as.character(Phe_WGS$FID)))
Clustered_ID_LPK$Group <- ifelse(Clustered_ID_LPK$V1==1,"LowTox","HighTox")
Clustered_ID_LPK$WGS_sam <- which(Clustered_ID_LPK$V2 %in% WGS_sample$V2)
Clustered_ID_LPK$V2 <- as.character(Clustered_ID_LPK$V2)
Clustered_ID_LPK$WGS_sam <- WGS_Sample_Reordered[which(Clustered_ID_LPK$V2 %in% WGS_Sample_Reordered$V2),][,1]

#################################################################################################
#################################################################################################


NPK_cluster <- kmeans ((my_data_2 [,5:6]), 2)
NPK_cluster
...
sort(table(NPK_cluster$clust))
clust_4 <- names(sort(table(NPK_cluster$clust)))

row.names(my_data_2[NPK_cluster$clust==clust_4[1],])
row.names(my_data_2[NPK_cluster$clust==clust_4[2],])
...
points(NPK_cluster$centers, col =1:2, pch =15, cex=3)
text (my_data_2[,5],my_data_2[,6],labels  = Phe_WGS$Neutrophiles_CTC, pos = 3,cex=0.75)
Clustered_ID_NPK<- as.data.frame(cbind(NPK_cluster$cluster, as.character(Phe_WGS$FID)))
Clustered_ID_NPK$Group <- ifelse(Clustered_ID_NPK$V1==1,"LowTox","HighTox")
Clustered_ID_NPK$WGS_sam <- which(Clustered_ID_NPK$V2 %in% WGS_sample$V2)
Clustered_ID_NPK$V2 <- as.character(Clustered_ID_NPK$V2)
Clustered_ID_NPK$WGS_sam <- WGS_Sample_Reordered[which(Clustered_ID_NPK$V2 %in% WGS_Sample_Reordered$V2),][,1]
